...
#define CPUS_RST_CTRL_REG (CPUCFG_REG_BASE+0)
#define CPU_CLK_GATING_REG (CPUCFG_REG_BASE+0x144)
#define GEN_CTRL_REG (CPUCFG_REG_BASE+0x184)

void cpu_l2_enable()
{
unsigned int reg=(1<<8)|(1<<0);

reg=readl(GEN_CTRL_REG);
reg=0;
writel(GEN_CTRL_REG,reg);
reg=0x10;
writel(GEN_CTRL_REG,reg);
reg=(1<<8)|(1<<0);
writel(CPU_CLK_GATING_REG,reg);
}
...
unsigned int reg;
	...
	reg=readl(SUNXI_SRAMC_BASE + 0x24);
	reg|=(1 << 15);
	writel(SUNXI_SRAMC_BASE + 0x24,reg);
	...
	printf("===== CCU ARM TEST APP FOR EZPI-A1 ======\r\n");
...
printf("ddr_ctrl2=%x\n",myval);
myval=readl(DRAM_CFG_REG);
printf("dram_ctrl2=%x\n",myval);

myval=readl(MBUS_RST_REG);
printf("MBUS_RST_REG=%x\n",myval);

myval=readl(MBUS_CLK_REG);
printf("MBUS_CLK_REG=%x\n",myval);
	memtest();
