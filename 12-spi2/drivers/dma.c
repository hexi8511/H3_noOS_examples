...
/**
*DMA mode configurate
*0 wait mode, 1 handshake mode
*/
void dma_cfg_src_mode(unsigned char channel,unsigned char mode)
{
	unsigned int val=readl(DMA_MODE_REG(channel));
	printf("DMA mode %x\n",readl(DMA_MODE_REG(channel)));
	val&=~4;
	val|=mode;
	writel(val,DMA_MODE_REG(channel));
	printf("DMA mode %x\n",readl(DMA_MODE_REG(channel)));
}

void dma_cfg_dst_mode(unsigned char channel,unsigned char mode)
{
	unsigned int val=readl(DMA_MODE_REG(channel));
	printf("DMA mode %x\n",readl(DMA_MODE_REG(channel)));
	val&=~8;
...
void dma_main(void)
{
	...
	dma_transmit_start(1,0x00000000,0x40000000,512);
