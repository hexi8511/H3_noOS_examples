...
static void init_cpus(void)
{
	...
	value = readl(SUNXI_PRCM_BASE + 0x0); //R_CPUS_CFG_BASE
	value &= ~1;
	writel(value, SUNXI_PRCM_BASE + 0x0);
	value = readl(SUNXI_PRCM_BASE + 0x0);
	value |= 1;
	writel(value, SUNXI_PRCM_BASE + 0x0);
}
...
void sys_clk_init()
{
	...
	if(0!=pll_set_out_freq(PLL_TYPE_DDR,672000000))//800MHz
		{
