...
{
	int err;
	u32 mult, freq,csize2;
	u32 cmult, csize, capacity;
	struct mmc_cmd cmd;
	char ext_csd[512];
	int timeout = 1000;
	...
	if (mmc->high_capacity) {
		...
		//csize2=( 0x1d177f80 >> 16);
		//msg("csize2:%d\n",csize2);
		csize=( (mmc->csd[2] & 0xffff0000) >> 16)|((mmc->csd[1] & 0x3f) << 16);
	...
	//msg("ss10  read_bl_len=%x ,capacity%x\n",mmc->read_bl_len,mmc->capacity);
...
//	msg("ss14\n");
	...
	//msg("ss17 read_bl_len=%x ,capacity%x\n",mmc->read_bl_len,mmc->capacity);

	mmc->lba = mmc->capacity>>9;///mmc->read_bl_len;
	//msg("ss18 %x\n",	mmc->lba);
#if 0
	if(!IS_SD(mmc)){
		switch(mmc->version)
