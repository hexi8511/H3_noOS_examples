...
#include "asm/io.h"
//#include "stdio.h"
//extern void memset(void*buf,int val,int size);
...
static int mmc_clk_io_onoff(int sdc_no, int onoff)
{
	...
	u32 gpiog_base = 0x01c20800 + 0x24*6;
//	u32 pll5_base = CCMU_PLL5_CLK_BASE;
	...
	switch (sdc_no) {
        case 0:
			...
			break;
		case 1:
            /* D1-PG3, D0-PG2, CLK-PG0, CMD-PG1, D3-PG4, D4-PG5 */
            if (onoff) {
	            writel(0x222222, gpiog_base + 0x0);
	            writel(0x555, gpiog_base + 0x1c);
	            writel(0xaaa, gpiog_base + 0x14);
	...
	/* config mod clock */
	if (onoff) {
		writel(0x81100100, mmchost->mclkbase);//phr0 600
		mmchost->mclk = 600000000;
	} else {
